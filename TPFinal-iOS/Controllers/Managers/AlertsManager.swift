...
class AlertsManager {
    ...
    // **
    // ** MARK : BASIC ALERT FOR UPGRADE ELEMENT
    // **
    func showMapConfirmation(title: String, message: String, closure: @escaping () -> ()) {
     
     let alert = UIAlertController(title: title, message: message, preferredStyle: .alert)
     
     let nextAction = UIAlertAction(title: NSLocalizedString("btn_yes", comment: ""), style: .default, handler: { action in
        closure()
     })
     
     let cancelAction = UIAlertAction(title: NSLocalizedString("btn_no", comment: ""), style: .cancel, handler: nil)
    ...
    func showAboutActions() {
        ...
        let rateAction = UIAlertAction(title: NSLocalizedString("txt_rateme", comment: ""), style: .default, handler: { action in
            if(AppDelegate.APP_STORE_URL != "") {
                ...
                self.showAlert(title: NSLocalizedString("title_error", comment: ""), message: NSLocalizedString("error_app_publish", comment: ""))
            }
        })
        ...
        let othersAppsAction = UIAlertAction(title: NSLocalizedString("txt_others", comment: ""), style: .default, handler: { action in
            if(AppDelegate.APP_DEVELOPER_URL != "") {
                ...
                self.showAlert(title: NSLocalizedString("title_error", comment: ""), message: NSLocalizedString("error_other_description", comment: ""))
            }
        })
